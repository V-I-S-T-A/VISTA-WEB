...
const LAST_LOGINS = [
  'June 6, 2026', 'June 5, 2026', 'June 4, 2026', 'June 3, 2026',
  'June 2, 2026', 'June 1, 2026', 'May 31, 2026', 'May 30, 2026',
]
...
function StatusBadge({ status }) {
  ...
  return (
    ...
    >
      <span style={{ backgroundColor: config.dot }} className="h-2 w-2 rounded-full flex-shrink-0" />
      {status}
    </span>
  )
}
  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        style={{ border: '1.5px solid #d1d5db', borderRadius: '7px', padding: '7px 11px', fontSize: '12px' }}
      >
        {value}
        <ChevronDown
      ...
      {isOpen && (
        <div role="listbox" className="absolute left-0 top-full z-10 mt-1.5 w-full overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg">
          {options.map((option) => {
...
export default function RecentSubmissionsTable() {
  ...
  async function handleSaveEdit(data) {
    console.log('Edited user payload:', data)
    // await supabase.from('tbl_Users').update({ ...data }).eq('user_id', data.user_id)
  }
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-gray-200 bg-white mx-4 sm:mx-6 lg:mx-8 my-4">
      ...
      <div className="bg-[#eef1f7] border-b border-gray-200 px-4 py-3">
        <div className="flex items-center justify-between gap-4 h-auto">
          <div
            ...
            style={{ width: '480px', paddingTop: '18px', paddingBottom: '18px', paddingLeft: CONTENT_PADDING }}
          >
            <Search
            ...
            <input
              ...
              style={{ height: '40px', border: '1.5px solid #d1d5db', borderRadius: '10px', padding: '0 16px 0 48px', fontSize: '13px' }}
            />
          </div>

          <div className="flex items-center gap-2.5" style={{ paddingRight: '20px' }}>
            <FilterDropdown label="Filter by role" options={ROLE_OPTIONS} value={roleFilter} onChange={(v) => { setRoleFilter(v); setCurrentPage(1) }} />
            <FilterDropdown label="Filter by status" options={STATUS_OPTIONS} value={statusFilter} onChange={(v) => { setStatusFilter(v); setCurrentPage(1) }} />
            <button
              type="button"
              ...
              style={{ border: '1.5px solid #d1d5db', borderRadius: '7px', padding: '7px 13px', fontSize: '12px' }}
            >
      ...
      {/* Section header */}
      <div className="bg-[#1f5cae] h-16 flex items-center" style={{ paddingLeft: CONTENT_PADDING }}>
        <h3 className="font-inter text-[18px] font-bold text-white">Recent Submissions</h3>
      </div>
          ...
          <tbody>
            {paginatedUsers.length === 0 ? (
              ...
              paginatedUsers.map((user) => (
                <tr key={user.id} className="h-16 border-b border-gray-100 transition-colors last:border-b-0 hover:bg-[#f7f9ff]">
                  <td className="px-5 py-2.5" style={{ paddingLeft: CONTENT_PADDING }}>
                    <div className="flex items-center gap-3">
                      <img src={user.avatar} alt="" className="flex-shrink-0 rounded-full object-cover" style={{ width: '45px', height: '45px' }} aria-hidden="true" />
                  ...
                  <td className="px-5 py-2.5">
                    <span
                      className={`inline-flex items-center justify-center rounded-full px-7 py-3 font-inter font-semibold ${user.role === 'Staff' ? 'bg-[#dfe7fb] text-[#12345b]' : 'bg-[#e8e3ff] text-[#4a3f99]'}`}
      ...
      <div
        ...
        style={{ paddingLeft: CONTENT_PADDING, paddingRight: CONTENT_PADDING, paddingTop: '12px', paddingBottom: '12px' }}
      >
        <p className="font-inter text-[14px] font-medium text-gray-500">
          Showing <span className="font-semibold text-gray-700">{paginatedUsers.length}</span> of{' '}
        ...
        {showPagination && (
          <div className="flex items-center gap-2">
            <button
              ...
              style={{ height: '30px', padding: '0 14px', fontSize: '13px', borderColor: '#d1d5db', backgroundColor: '#f9fafb', color: safeCurrentPage === 1 ? '#9ca3af' : '#374151', cursor: safeCurrentPage === 1 ? 'not-allowed' : 'pointer' }}
            >
              Previous
            </button>
            {pageNumbers.map((page) => (
              <button
                ...
                className="font-inter font-semibold border rounded-md transition"
                style={{ width: '34px', height: '30px', fontSize: '13px', borderColor: page === safeCurrentPage ? '#002b5c' : '#d1d5db', backgroundColor: page === safeCurrentPage ? '#002b5c' : '#ffffff', color: page === safeCurrentPage ? '#ffffff' : '#374151' }}
              >
                {page}
              </button>
            ))}
            <button
              ...
              className="font-inter font-semibold border rounded-md transition"
              style={{ height: '30px', padding: '0 14px', fontSize: '13px', borderColor: '#d1d5db', backgroundColor: '#ffffff', color: safeCurrentPage >= totalPages ? '#9ca3af' : '#374151', cursor: safeCurrentPage >= totalPages ? 'not-allowed' : 'pointer' }}
            >
              Next
            </button>
          </div>
